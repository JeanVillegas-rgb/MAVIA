...
class PrerequisiteEdge(models.Model):
    ...
    class Signal(models.TextChoices):
        TITLE_REFERENCE = "title_reference", "Dependent text names the prerequisite's concept"
        SECTION_REFERENCE = "section_reference", "Dependent's section names the prerequisite's concept"
        TERM_COOCCURRENCE = "term_cooccurrence", "Shared salient terms introduced by the prerequisite"
        CHUNK_CONTINUATION = "chunk_continuation", "Next part of a passage the chunker split"
        DEFINITION_SCOPE = "definition_scope", "Opening definition the later definitions build on"

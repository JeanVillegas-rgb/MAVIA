...
class EnrollmentAndProgressApiTests(APITestCase):
    ...
    def test_enroll_then_progress_shows_not_started(self):
        ...
        resp = self.client.post(
            f"/api/adaptive-portal/courses/{self.course.id}/enrollments/",
            {"student_id": self.student.id},
        )
        self.assertEqual(resp.status_code, 201)

        progress = self.client.get(f"/api/adaptive-portal/courses/{self.course.id}/progress/")
    ...
    def test_start_requires_enrollment(self):
        ...
        resp = self.client.post("/api/adaptive-portal/start/", {"course_id": self.course.id})
    ...
    def test_student_lesson_read_endpoints(self):
        ...
        courses = self.client.get("/api/adaptive-portal/my-courses/")
        ...
        lessons = self.client.get(
            f"/api/adaptive-portal/my-courses/{self.course.id}/lessons/"
        )
        self.assertEqual(lessons.status_code, 200)
        lesson_id = lessons.data[0]["id"]

        package = self.client.get(f"/api/adaptive-portal/lessons/{lesson_id}/")
    ...
    def test_full_student_flow_updates_progress(self):
        ...
        start = self.client.post("/api/adaptive-portal/start/", {"course_id": self.course.id})
